	...
	<div class="container body-wrapper">
		<section class="posts-dispaly-page col-md-8">
			<h2 class="text-center">Students Forum</h2>

			<div class="new-post col-xs-12">
				<?php echo form_open_multipart('StudentForum/doupload');?>
			...
			$totrec=8;
			$data1=$this->db->query('SELECT * FROM discussions where `type_id`=2 and `discussion_id` = `parent_discussion_id` and `status`=1 order by date desc');
			$data=$this->db->query("SELECT * FROM `discussions` where `type_id`=2 and `discussion_id` = `parent_discussion_id` and `status`=1 order by `date` desc LIMIT $sind,8 ");
			...
			foreach ($data->result() as $row)
			{
				...
				$cntviews=$countviews->num_rows();
					...
					<h3>';
					echo substr($row->discussion_name,4,100);
					echo'</h3><p>';
					...
					<div class="details col-md-12">
						...
						<div class="likes detail liked" ><i class="fa fa-thumbs-o-up" onclick="update_like(';
			...
			for($i=0;$i<$pages;$i++)
			{
				...
				if($index==$sind)
				{
					echo "<a href = '".base_url()."StudentForum/StudentForumPage/$index' ><li style='color:white;background-color:black'>". $pn."</li></a>";
				}
				else 	echo "<a href = '".base_url()."StudentForum/StudentForumPage/$index'><li>". $pn."</li></a>";
			
			}
			if($nind<$tabrec)
			{
				echo "&nbsp;<a href = '".base_url()."StudentForum/StudentForumPage/$nind' aria-label='Next'>";
